...
- README, plan, checklist, ticket, decision log, incidents log, and metrics tracker exist under `docs/tests/3-integration-tests/` and reflect the latest helper/layout/command expectations.
...
- Upcoming Phase 4 (Redis nightly CI coverage) is planned but **blocked until the contract test kit under `docs/tests/4-contract-tests/**`ships**; once that milestone lands we’ll add an overnight GitHub Actions job that exports`ENABLE_REDIS_INTEGRATION=true`, captures artifacts, and tracks performance in `metrics-tracker.md`.
...
- Dependent docs: `docs/tests/overhaul/**`, `docs/tests/test-classification-2025-12-22.md`, `.github/workflows/backend-ci.yml`, `docs/ci-cd/backend/**`.
