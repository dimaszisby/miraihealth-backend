...
- Detailed references:
  - `docs/security/guides/README.md`
...
12. For production release PRs, follow `docs/security/guides/security-release-delta-sop.md` and update the active security audit run docs.
...
- If you skip a test due to timeline pressure, note it in the PR and create a TODO in `docs/todos/`.
...
5. **CI parity**  
   The `tests` and `contract_local` jobs already set `DISABLE_RATE_LIMITING=true` and extract the same token in `backend-ci.yml`. If a contract job fails, inspect the artifacts under `docs/tests/4-contract-tests/**`, review `tmp/backend-contract.log`, and mirror `npm run contract:local:full` locally (adjust `CONTRACT_LOCAL_PORT` if needed) for parity.
...
- **Docs**: `docs/tests/**`, `docs/ci-cd/backend/**`, `docs/development/architecture/**`.
- **Owner**: Backend Platform (@dimaspramudya). Reach out for CI failures or infrastructure issues.
- **Incident logging**: If an issue reaches `main`/production, record it in `docs/incidents/` with root cause and remediation steps.
