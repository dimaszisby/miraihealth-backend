...
- [ ] Workflow is documented in:
  ...
  - [ ] `docs/ci-cd/backend/GITHUB_ACTIONS_WORKFLOW_GUIDELINES.md`
...
- [ ] `"test:unit": "…"`.
- [ ] `"test:integration": "…"`.
- [ ] `"test:contract:local": "node tests/contract/postman-newman/scripts/run-contract-local.js"` (or equivalent).
- [ ] `"test:contract:staging": "node tests/contract/postman-newman/scripts/run-contract-staging.js"`.
- [ ] `"db:migrate:test": "…"`.
- [ ] `"start:test": "…"`.
- [ ] `"format:check": "…"`.
- [ ] `"docs:openapi:check": "…"`.
...
- [ ] `docs/tests/4-contract-tests/postman-newman/PLAN.md` and `PIPELINE_OVERVIEW.md` describe CI usage (local + staging).
...
- [ ] Contract runs produce:
  - [ ] JUnit XML reports.
  - [ ] HTML reports.
  - [ ] Saved under `tests/contract/postman-newman/reports/local/**` and `reports/staging/**`.
...
- [ ] A known green run on `dev` or `main` is referenced in:
  - [ ] `docs/tests/4-contract-tests/postman-newman/CHECKLIST.md` (as a baseline).
