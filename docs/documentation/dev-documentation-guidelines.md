...
Create a predictable, portfolio-grade document kit for every major engineering topic (tests, CI, infra, migrations, etc.). Each kit lives inside `docs/<domain>/<topic>/` (e.g., `docs/tests/1-static-checks/`) and mirrors the structure completed under `docs/tests/overhaul/`.
...
- Place the kit under the closest domain folder: tests (`docs/tests`), architecture (`docs/development/architecture`), CI (`docs/ci-cd`), etc.
- If a topic spans multiple domains, host primary docs where the owning team lives and link out to supporting folders (e.g., CI doc referencing `docs/tests/1-static-checks/`).
- Reference paths relative to repo root in cross-links for portability (`[link](docs/tests/overhaul/test-structure-plan.md)`).
...
3. **Draft the plan** (phases, dependencies, guardrails). Reuse content blocks from `docs/tests/overhaul/test-structure-plan.md`.
...
Adhering to this guide ensures each future topic—static checks, observability, feature migrations—ships with the same high-quality documentation kit showcased in `docs/tests/overhaul/`, reinforcing both team DX and portfolio polish.
