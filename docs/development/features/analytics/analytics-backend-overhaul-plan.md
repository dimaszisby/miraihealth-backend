...
- **Contract versioning:** `/analytics/dashboard` now accepts optional query parameter `v` (default `1`, send `2` for lifecycle metadata). Documented in `docs/openapi/lakira-backend-openapi.json`.
...
- **OpenAPI refresh:** `docs/openapi/lakira-backend-openapi.json` regenerated via `npm run docs:openapi:generate`, ensuring the published spec reflects the v2 dashboard contract.
