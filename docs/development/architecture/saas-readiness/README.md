...
The audit grades against the project's own intended standard (`.claude/rules/*`, `docs/tests/TESTING_STRATEGY.md`, `docs/ci-cd/CI_CD_STRATEGY.md`) and not just generic SaaS criteria. Empirical verification commands are run; a category cannot be ✅ if its verification fails or if the intended standard itself is missing.
...
Doc kits for each P0/P1 remediation phase live as siblings under `docs/development/architecture/<topic>/`. The full per-phase mapping (which kit closes which audit gaps, gating ADRs, recommended execution order) is in [`iteration-plan.md`](./iteration-plan.md). The `saas-readiness/` folder stays as the **tracker**; the actual remediation work lives in the per-topic kits.
...
- `docs/todos/2026-05-01-promt-saas-readiness-audit.md` — the audit prompt.
- `.claude/rules/` — the intended standard.
- `docs/tests/TESTING_STRATEGY.md`
- `docs/ci-cd/CI_CD_STRATEGY.md`
- `docs/security/` — existing security-audit infrastructure (do not reformat).
- `docs/openapi/lakira-backend-openapi.json` — generated API contract.
