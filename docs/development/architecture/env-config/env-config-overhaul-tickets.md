...
- Status: ✅ Helper added, analytics tests migrated, and as of 2026-01-05 the ESLint override blocks direct `process.env` usage. Documentation lives in `docs/development/architecture/env-config/with-test-env-helper.md`. Follow-up ticket: TKT-005A to clean up legacy suites (formatting + helper adoption evidence).

## TKT-005A — Legacy Test Cleanup

- Scope: Update legacy `__tests__/**` suites to import Jest globals, use `withTestEnv` (or `getEnv`) instead of mutating `process.env`, and clear lingering lint errors. Details captured in `docs/development/architecture/env-config/legacy-test-cleanup.md`.
- Deliverables:
...
- Archive results (metrics, retros, postmortems) in `docs/development/architecture/env-config` after completion.
