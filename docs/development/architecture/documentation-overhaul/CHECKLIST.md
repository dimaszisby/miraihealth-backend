...
- [x] `git mv docs/tests/4-contract-tests/postman-newman/{collections,environments,scripts} tests/contract/postman-newman/`
- [x] `git mv docs/tests/4-contract-tests/schemathesis/{scripts,requirements.txt} tests/contract/schemathesis/`
...
- [x] Swept the dotted module path — one extra hit beyond the plan:
      **`docs/ci-cd/CI_CD_DEVELOPER_SIMPLIFIED_GUIDE.md:155`** carried a copy-pasteable
...
- [x] **Not in the plan:** internal path math. Both script families compute `repoRoot` by
      ...
      `specPath` still points at `docs/openapi/` — correct until Phase 3.
...
| `npm run test:unit:security-framework`        | ✅ 1 suite / 8 tests                                               |
| `npm run docs:openapi:check`                  | ✅ 0 — generator writes to `docs/openapi/`, CI diffs the same path |
...
resolves `docs/openapi/lakira-backend-openapi.json`, and the runner aborts on `fs.access` if the
...
- [ ] `development/run-postgres-in-docker.md` ← `docker/postgres-docker-guide.md`
      (fix its dead reference to `docs/analytics/...`)
...
- [ ] `docs/todos/2026-06-04-*.md` carries an unchecked "2026-07-02 review" item, now ~6 weeks
      overdue.
