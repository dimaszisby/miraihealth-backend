...
- ESLint now lint checks `__tests__/**`, but documentation under `docs/development/architecture/env-config/**` still claims a `no-restricted-properties`/`no-process-env` guardrail exists even though `eslint.config.mjs` never enforces it. This gap can confuse reviewers and weakens the “industry-standard” story in interviews.
...
- [ ] Record lint runtime/observations in `docs/tests/1-static-checks/metrics-tracker.md` (and any relevant ticket/checklist files) to preserve evidence for portfolio reviews.
