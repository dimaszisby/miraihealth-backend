...
Local regen DOES change docs/openapi/lakira-backend-openapi.json, but git commit fails:
...
1. Add scripts/normalize-openapi.ts:
   - Read docs/openapi/lakira-backend-openapi.json
...
2. Update package.json:
   ...
   - docs:openapi:check remains:
     npm run docs:openapi:generate && git diff --exit-code docs/openapi/lakira-backend-openapi.json

3. Fix lint-staged config:
   - Either exclude docs/openapi/\*.json from lint-staged formatting
     (recommended since it’s generated and normalized),
     OR ensure prettier config matches canonical output exactly.

4. Add .gitattributes:
   docs/openapi/\*.json text eol=lf

5. IMPORTANT CHECK FIRST:
   - Inspect current lint-staged config (package.json or .lintstagedrc)
   - Identify which task runs on docs/openapi/\*.json and causes reversion.
...
- [x] Added `scripts/normalize-openapi.ts` that reuses the repo’s Prettier config to format `docs/openapi/lakira-backend-openapi.json` (no more husky conflicts).
- [x] `npm run docs:openapi:generate` now chains the generator and normalizer so local runs match CI output.
- [x] `.gitattributes` forces LF endings for `docs/openapi/*.json` to avoid cross-platform diffs.
