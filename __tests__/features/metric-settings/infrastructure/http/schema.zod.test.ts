...
describe("metric-settings schemas", () => {
  ...
  it("rejects enabling time frame without both dates", () => {
    ...
    if (!result.success) {
      ...
      expect(fields).toEqual(expect.arrayContaining(["startDate", "deadlineDate"]));
    }
  });
  ...
  it("requires goal fields when enabling via partial update", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues.some((issue) => issue.path.includes("goalType"))).toBe(true);
      expect(result.error.issues.some((issue) => issue.path.includes("goalValue"))).toBe(true);
    }
  });
