...
import { toListDomain } from "../infrastructure/mappers/MetricCategoryMapper";
import {
 ...
 */
export const createMetricCategoryServiceLegacy = async (
  userId: string,
  ...
  return toDomainLegacy(category);
};
 ...
 */
export const getUserMetricCategoryByIdServiceLegacy = async (
  userId: string,
  categoryId: string
): Promise<MetricCategoryDomain> => {
...
export async function listMetricCategoriesLegacy({
  userId,
  limit,
  sort,
  q,
  ...
  const items = toListDomainLegacy(itemsRaw);
 ...
 */
export const updateMetricCategoryServiceLegacy = async (
  userId: string,
  categoryId: string,
 ...
 */
export const deleteMetricCategoryServiceLegacy = async (
  userId: string,
  categoryId: string
): Promise<MetricCategoryDomain> => {
  const category = await findOwnedCategory(userId, categoryId);
 ...
 */
export const generateDummyCategoriesServiceLegacy = async (
  userId: string,
  count: number
): Promise<MetricCategoryDomain[]> => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    dummyCategories.push(toDomainLegacy(category));
  }
