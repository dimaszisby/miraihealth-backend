...
When upstream fixes are unavailable, document compensating controls (feature flags, WAF rules) in `documents/security/audit/` and open backlog issues.
...
- Document categorization in the relevant checklist/todo (e.g., `documents/todos/2026-01-19-todo-github-actions-openapi.md` Section 5).
...
- If forced downgrades or patch forks are necessary, log a decision entry in `documents/security/audit/security-audit-log-baseline-simple.md` (or a dedicated `decisions.md` once the dependency topic expands).

## Exceptions

- When upstream fixes are not available:
  - Add temporary suppression note in `documents/security/DEPENDENCY_POLICY.md` (this file) or a companion log.
...
- Security audit artifacts: `documents/security/audit/`.
