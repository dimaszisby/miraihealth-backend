...
2. **Deterministic staging fixtures**
   - Staging database seeded with the IDs from `documents/tests/4-contract-tests/seed-strategy.md`.
...
2. `contract_staging`
   ...
   - Reports saved under `documents/tests/4-contract-tests/postman-newman/reports/staging/<timestamp>/` and uploaded as `newman-contract-staging`.
...
2. Run:
   ...
   node documents/tests/4-contract-tests/postman-newman/scripts/run-contract-staging.js
   ```

3. Inspect `documents/tests/4-contract-tests/postman-newman/reports/staging/<timestamp>/` for HTML/JUnit outputs.
