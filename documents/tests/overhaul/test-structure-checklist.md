...
- [x] Review `documents/tests/test-classification-2025-12-22.md` with feature owners; adjust any disputed rows.
- [x] Sign off the move map in `documents/tests/test-structure-move-plan-2025-12-22.md`.
- [x] Decide whether `__tests__/integration/helpers/test-utils.ts` stays put or moves into a shared test-support package.
- [x] Document env prerequisites for integration runs (see `documents/tests/3-integration-tests/README.md` for DB/Redis expectations).
...
- [x] Create Phase 2 tracker (`documents/tests/overhaul/phase2-integration-coverage.md`) listing targets + status.
...
- [x] Refresh `documents/tests/2-unit-tests` and `documents/tests/3-integration-tests` to describe the new layout, scripts, and expectations.
...
- [x] Update `documents/ci-cd/backend/**` guidance to reference the new scripts and folder structure.
