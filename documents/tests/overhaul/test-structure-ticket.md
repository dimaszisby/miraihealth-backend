...
- `documents/tests/test-classification-2025-12-22.md` shows 27 suites are pure unit tests while only 7 hit the HTTP stack or DB.
...
- `package.json` scripts match the recommended block from `documents/tests/test-tooling-recommendations-2025-12-22.md`, and `npm run test` executes unit + integration sequentially.
...
- Documentation under `documents/tests/2-unit-tests`, `documents/tests/3-integration-tests`, and `documents/ci-cd/backend/**` references the new layout/scripts.
