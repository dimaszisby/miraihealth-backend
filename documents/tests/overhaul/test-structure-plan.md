...
- Current layout under `__tests__/` is feature- and layer-aware but mixes unit and integration suites (see `documents/development/architecture/test/test-structure/test-structure-concern.md`).
...
2. Track candidates and status in `documents/tests/overhaul/phase2-integration-coverage.md`, then for each:
...
1. Update `documents/tests/2-unit-tests` and `documents/tests/3-integration-tests` to describe the new folder layout, tooling, and expectations.
...
1. Update backend CI workflow(s) (`documents/ci-cd/backend/**`, GitHub Actions) to run:
