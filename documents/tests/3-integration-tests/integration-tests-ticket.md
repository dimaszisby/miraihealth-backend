...
- CI/CD docs (`documents/ci-cd/backend/**`) call for integration tests to gate every PR before contract suites; this ticket tracks the work required to make that stage demonstrably ready.

## Acceptance Criteria

- README, plan, checklist, ticket, decision log, incidents log, and metrics tracker exist under `documents/tests/3-integration-tests/` and reflect the latest helper/layout/command expectations.
...
- Upcoming Phase 4 (Redis nightly CI coverage) is planned but **blocked until the contract test kit under `documents/tests/4-contract-tests/**`ships**; once that milestone lands we’ll add an overnight GitHub Actions job that exports`ENABLE_REDIS_INTEGRATION=true`, captures artifacts, and tracks performance in `metrics-tracker.md`.
...
- Dependent docs: `documents/tests/overhaul/**`, `documents/tests/test-classification-2025-12-22.md`, `.github/workflows/backend-ci.yml`, `documents/ci-cd/backend/**`.
