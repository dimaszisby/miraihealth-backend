...
- CI/CD docs (`documents/ci-cd/backend/**`) call for unit tests to guard every PR before integration or contract jobs run; this ticket closes the documentation/process gap.

## Acceptance Criteria

- README + workflow guidelines + plan + checklist + ticket + metrics + decision log + incident log exist under `documents/tests/2-unit-tests/` and describe scope, commands, KPIs, and governance.
...
- Dependent docs: `documents/tests/overhaul/**`, `documents/ci-cd/backend/**`, `documents/tests/1-static-checks/**`.
