...
Create a predictable, portfolio-grade document kit for every major engineering topic (tests, CI, infra, migrations, etc.). Each kit lives inside `documents/<domain>/<topic>/` (e.g., `documents/tests/1-static-checks/`) and mirrors the structure completed under `documents/tests/overhaul/`.
...
- Place the kit under the closest domain folder: tests (`documents/tests`), architecture (`documents/development/architecture`), CI (`documents/ci-cd`), etc.
- If a topic spans multiple domains, host primary docs where the owning team lives and link out to supporting folders (e.g., CI doc referencing `documents/tests/1-static-checks/`).
- Reference paths relative to repo root in cross-links for portability (`[link](documents/tests/overhaul/test-structure-plan.md)`).
...
1. **Create folder skeleton**
   ...
   touch documents/<domain>/<topic>/{README.md,<topic>-plan.md,<topic>-checklist.md,<topic>-ticket.md,decisions.md,incidents.md}
   ```
2. **Populate README first** so teammates know why the folder exists.
3. **Draft the plan** (phases, dependencies, guardrails). Reuse content blocks from `documents/tests/overhaul/test-structure-plan.md`.
...
Adhering to this guide ensures each future topic—static checks, observability, feature migrations—ships with the same high-quality documentation kit showcased in `documents/tests/overhaul/`, reinforcing both team DX and portfolio polish.
