...
- Updated `documents/development/features/README.md` with the standardized router factory pattern so new slices follow the same structure.
...
- Captured the finalized bootstrap flow in `documents/development/architecture/orm-bootstrap.md` and linked it from the feature README so onboarding stays consistent.
...
- Captured the detailed follow-up plan in `documents/development/architecture/metric-category-migration-plan.md`, outlining domain/application/infrastructure tasks required to replace the legacy services.
...
|------|---------------------------|----------------|
| `src/routes` | *(removed)* | Folder deleted after migrations; lint guard now blocks imports from `src/routes/**`. |
| `src/controllers` | *(removed)* | Folder deleted; future controllers must live inside feature slices. |
...
| Path aliases | *(none pending)* | `@routes/*`, `@controllers/*`, and `@services/*` have been removed; configs now rely on feature paths. |
