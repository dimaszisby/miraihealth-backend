...
- [ ] `npm run docs:openapi:generate`; verify `git diff -- documents/openapi/` is empty (no public-path drift).
...
- [ ] Audit `documents/openapi/` references for any path that mentions `src/features/<feature>/...` and refresh.
