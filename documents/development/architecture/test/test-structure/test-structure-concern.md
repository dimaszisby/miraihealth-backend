...
- The existing test documentation under `documents/tests/**` talks about **static checks, unit, integration, contract**, but the code structure under `__tests__` does not reflect those categories.
...
Under `documents/tests/` we already have conceptual test types:
...
There are also CI/CD docs under `documents/ci-cd/backend/**` that assume separate CI jobs / commands for:
...
- **Contract tests**
  - Remain in `documents/tests/4-contract-tests/postman-newman/**`.
...
- Update `documents/tests/2-unit-tests/*` and `documents/tests/3-integration-tests/*` to:
...
- Ensure job names and scripts align with this document and the `documents/ci-cd/backend/**` docs.
...
- After Codex’s feedback, I will:
  - Update this document and the docs under `documents/tests/**`.
